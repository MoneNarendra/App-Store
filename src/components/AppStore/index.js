...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, userSearch: ''}
  ...
  getSearchResult = () => {
    ...
    const searchResult = appsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(userSearch.toLowerCase()),
    )
    return searchResult
  }

  filterApps = searchResult => {
  ...
  render() {
    ...
    const filteredApps = this.filterApps(searchResult)

    return (
      <div className="appstore-bg-container">
        <div className="appstore-inner-container">
          ...
          <div className="tab-itemes-container">
            <div className="tab-itemes-inner-container">
              <ul className="tab-itemes-list">
              ...
              <hr className="tab-names-line" />
            </div>
          </div>
          <div className="app-list-constiner">
            <ul className="app-list-items">
              {filteredApps.map(eachApp => (
                <AppItems eachApp={eachApp} key={eachApp.appId} />
              ))}
            </ul>
          </div>
        </div>
